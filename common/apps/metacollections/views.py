...
from .serializers import (
    ...
    AddToStarredSerializer,
    ShareCollectionSerializer,
    AddToLikedCollectionSerializer,
    MySharedCollectionsResponseSerializer,
    SharedCollectionsByUserResponseSerializer,
    UsersWhoSharedCollectionsListResponseSerializer
)
@extend_schema(
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if collection.view == 'public' or collection.user == request.user:
                ...
                return Response({'error': 'You do not have permission to favourite this collection'}, status=status.HTTP_403_FORBIDDEN)
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if collection.view == 'public' or collection.user == request.user:
                ...
                return Response({'error': 'You do not have permission to star this collection'}, status=status.HTTP_403_FORBIDDEN)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
@extend_schema(
    ...
    responses={200: OpenApiResponse(description='Collection shared.'), 400: OpenApiResponse(description='Invalid input.')},
    description="Share a collection with another user by email.",
...
class ShareCollectionView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if SharedCollection.objects.filter(collection=collection, owner=request.user, shared_with=user_to_share).exists():
                return Response({'error': 'Collection already shared with this user.'}, status=status.HTTP_400_BAD_REQUEST)
            SharedCollection.objects.create(collection=collection, owner=request.user, shared_with=user_to_share)
