...
def pair_tv(session_id: str, current_user: User = Depends(get_current_user)):
    ...
    access_token = create_access_token({"user_id": str(user_obj.id), "role": "tv"}, exp_minutes=TV_ACCESS_EXPIRE_MINUTES)
...
def refresh_token(refresh_token: str):
    ...
    for device in devices:
        if bcrypt.checkpw(refresh_token.encode(), device.refresh_token_hash.encode()):
            ...
            new_access = create_access_token({"user_id": str(device.user.id), "role": role}, exp_minutes=exp_minutes)
