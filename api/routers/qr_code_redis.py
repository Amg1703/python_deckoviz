...
# Admin endpoints
# @router.get("/qr/admin/rooms", dependencies=[Depends(get_current_user)])
# async def list_active_rooms(redis_manager: QRRedisManager = Depends(get_qr_redis_manager)):
#     """
#     List all active rooms.
#     Admin endpoint for monitoring.
#     """
#     rooms = redis_manager.get_all_rooms()
#     result = []
    
#     for room_id in rooms:
#         metadata = redis_manager.get_room_metadata(room_id) or {}
#         count = redis_manager.get_room_connection_count(room_id)
...
#             "has_tv": metadata.get("has_tv", False),
#             "has_mobile": metadata.get("has_mobile", False)
#         })
    
#     return {"active_rooms": result, "count": len(result)}

# @router.get("/qr/admin/room/{room_id}/messages", dependencies=[Depends(get_current_user)])
# async def get_room_message_history(room_id: str, limit: int = 50, redis_manager: QRRedisManager = Depends(get_qr_redis_manager)):
#     """
#     Get message history for a room.
#     Admin endpoint for monitoring.
#     """
#     messages = redis_manager.get_room_messages(room_id, limit)
#     metadata = redis_manager.get_room_metadata(room_id) or {}
...
# # Cleanup task for expired device IDs is handled by Redis TTL
# @router.on_event("startup")
# async def startup_event():
#     # With Redis, we don't need to explicitly handle cleanup
